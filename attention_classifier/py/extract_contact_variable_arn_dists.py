...
def contacts_per_pdb(contact_arn_dist, non_contact_arn_dist, same_sequence_ids, protein_data):
    ...
    for pdb_id in same_sequence_ids:
        calc_contact_sites2(contact_arn_dist,non_contact_arn_dist, pdb_id, protein_data, in_contact_sites, non_contact_sites, subset_non_contact_sites)
...
def calc_contact_sites(contact_arn_dist,non_contact_arn_dist, pdb_id, protein_data, in_contact_sites, non_contact_sites, subset_non_contact_sites):
    ...
    for i, residue1 in enumerate(chain):
        for j, residue2 in enumerate(chain):
            ...
            if residue1.id[1] > len(protein_data[pdb_id]) or residue2.id[1] > len(protein_data[pdb_id]):
                continue
            try:
                distance = float(abs(residue1['CA'] - residue2['CA']))
            except KeyError:
                continue
            aa_distance = int(abs(residue1.id[1] - residue2.id[1]))
            if distance < contact_arn_dist:
                if aa_distance > 2:
    #                 print(residue1.id[1], residue1.resname, residue2.id[1], residue2.resname, distance)
                ...
                if aa_distance > 2 and distance > non_contact_arn_dist:
